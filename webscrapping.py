    ...
    def gravar_informacoes(self, titulo_da_materia, texto_da_materia, dados_de_apresentacao_materia):
        ...
        for i in range(0, len(titulo_da_materia)):
            ...
            try:
                ...
                values((select IdVereador from nome_vereadores where NomeVereador||' '||SobreNome = '" + self.autor[
                    i] + "'),'" \
                                      + self.titulo[i] + "','" + self.texto_materia[i] + "','" + self.data_materia[
                                          i] + "','" + self.tipo_materia[i] + "','" \
